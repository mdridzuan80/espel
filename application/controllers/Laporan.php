...
class Laporan extends MY_Controller
{
    ...
    public function penganjur_kursus()
    {
        $this->applog->write(['nokp' => $this->appsess->getSessionData('username'), 'event' => 'Akses laporan Penganjuran Kursus']);
        $plugins = ['embedjs' => [$this->load->view('laporan/ptj/penganjuran_kursus/js', '', true)]];

        return $this->renderView("laporan/ptj/penganjuran_kursus/param", '', $plugins);
    }

    public function ajax_penganjur_kursus()
    {
        ...
        $this->applog->write(['nokp' => $this->appsess->getSessionData('username'), 'event' => 'Papar laporan Senarai Penganjuran Kursus']);

        return $this->load->view('laporan/ptj/penganjuran_kursus/result', $data);
    }

    public function ajax_penganjur_kursus_export()
    {
        $tahun = $this->input->post('tahun');
        $jenis = $this->input->post('jenis');
        ...
        $data['tahun'] = $this->input->post('tahun');
        $data['sen_kursus'] = $this->kursus_model->sen_kursus_selesai($this->appsess->getSessionData('ptj_jabatan_id'), date('Y'));
        $data['carta'] = $this->hrmis_carta->get($this->appsess->getSessionData('ptj_jabatan_id'));
        ...
        switch ($jenis) {
            case 1:
                try {
                    ...
                    $this->applog->write(['nokp' => $this->appsess->getSessionData('username'), 'event' => 'Export PDF laporan Senarai Peganjuran Kursus']);
                    $html2pdf->writeHTML($this->load->view("laporan/ptj/penganjuran_kursus/pdf", $data, true));
                    $html2pdf->output('about.pdf');
                } catch (Html2PdfException $e) {
                    $formatter = new ExceptionFormatter($e);
                    echo $formatter->getHtmlMessage();
            ...
            case 2:
                $this->output->set_header('Content-type: application/vnd.ms-excel');
                $this->output->set_header('Content-Disposition: attachment; filename=senarai_hadir.xls');
                $content = $this->load->view("laporan/ptj/penganjuran_kursus/pdf", $data, true);
                $this->applog->write(['nokp' => $this->appsess->getSessionData('username'), 'event' => 'Export Excel laporan Senarai Latihan yang dihadiri']);
                echo $content;
                break;
