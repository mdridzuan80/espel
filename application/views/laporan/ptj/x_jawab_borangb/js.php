...
$(function(){
    $('.btn-papar').on('click', function(e)
    {
        ...
        $.ajax({
            method: 'post',
            data: {tahun: tahun, jabatan: jabatan, kelas: kelas, skim: skim, gred: gred},
            url: base_url + 'laporan/ajax_x_jawab_borang_b',
    ...
    function janaReport(data, jenis){
        $.ajax({
            ...
            method: 'post',
            data: data,
            url: base_url + 'laporan/ajax_x_jawab_borang_b_export',
            success: function(blob){
                var ext = {1: '.pdf', 2: '.xls', 3: '.doc'};
                console.log(blob.size);
                var link=document.createElement('a');
                link.href=window.URL.createObjectURL(blob);
                link.download="tidak_menjawab_borangb" + ext[jenis];
                link.click();
            }
        });
    }
