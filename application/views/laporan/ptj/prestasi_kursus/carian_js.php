   ...
   var postData={};
    ...
    $("#comKelas").change(function(e){
        $.ajax({
            method: 'post',
            data: {'kelas[]':$(this).val()},
            url: base_url + 'api/get_laporan_skim2',
            success: function(gred,textStatus,jqXHR)
            {
                $('#comSkim').html('');
                for(var i=0;i<gred.length;i++)
                {
                    var option=$('<option></option>').attr("value",gred[i]['id']).attr("selected",'selected').text(gred[i]['kod']);
        ...
        $.ajax({
            method: 'post',
            data: {'kelas[]':$(this).val()},
            url: base_url + 'api/get_laporan_gred2',
            success: function(gred,textStatus,jqXHR)
            {
                $('#comGred').html('');
                for(var i=0;i<gred.length;i++)
    ...
    $("#comSkim").change(function(){
        $.ajax({
            method: 'post',
            url: base_url + 'api/get_laporan_gred2',
            data: {'kelas[]':$('#comKelas').val(), 'skim[]': $(this).val()},
            success: function(gred,textStatus,jqXHR)
            {
                $('#comGred').html('');
                for(var i=0;i<gred.length;i++)
    ...
    $('#cmdDoTapis').click(function(e){
        ...
        options.filter['hari[]'] = $('#comStatus').val();
